  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-pink-200 bg-gradient-to-r from-pink-50 via-purple-50 to-indigo-50 dark:from-pink-500/10 dark:via-purple-500/10 dark:to-indigo-500/10 dark:border-pink-500/20 shadow-sm">
      ...
      <div className="relative px-5 py-4">
        <button onClick={() => setDismissed(true)} className="absolute top-3 right-3 text-gray-400 hover:text-gray-600 transition-colors">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex-1 min-w-[200px]">
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100">Happy Birthday, Giridharan! 🎉</p>
            <p className="text-sm text-gray-600 dark:text-gray-300 mt-0.5">Wishing you a wonderful day from the entire team!</p>
          </div>
          {!cakeCutDone && (
            <button
        ...
        <div className="mt-3 pt-3 border-t border-pink-200/50">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-bold text-gray-800 dark:text-gray-200">Today&apos;s Birthdays</p>
          </div>
              ...
              return (
                <div key={person.userId} className="flex items-center gap-2.5 bg-white/60 dark:bg-white/10 rounded-xl px-3 py-2 border border-pink-100 dark:border-pink-500/20">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400">{person.designation}</p>
                  </div>
...
export default function BirthdaysReviewPage() {
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-4xl animate-fade-in">
      <div>
        <h1 className="text-xl font-bold text-gray-900 tracking-tight">Birthday UI Review</h1>
        <p className="mt-1 text-sm text-gray-400">Preview of all birthday-related components</p>
      </div>

      {/* 0. Cake Cutting Modal — direct test */}
      <section>
        <h2 className="text-sm font-bold text-gray-600 mb-3 uppercase tracking-wider">0. Cake Cutting Modal (direct test)</h2>
        <button
      ...
      <section>
        <h2 className="text-sm font-bold text-gray-600 mb-3 uppercase tracking-wider">1. Dashboard Banner (shows when someone has birthday today)</h2>
        <MockBirthdayBanner />
      </section>

      {/* 2. Sidebar Widget — Upcoming Birthdays */}
      <section>
        <h2 className="text-sm font-bold text-gray-600 mb-3 uppercase tracking-wider">2. Sidebar Widget (upcoming 7 days)</h2>
      ...
      <section>
        <h2 className="text-sm font-bold text-gray-600 mb-3 uppercase tracking-wider">3. Live Components (from API)</h2>
        <p className="text-xs text-gray-400 mb-3">These pull real data from the /users/birthdays endpoint</p>
        ...
        <p className="text-xs text-gray-400 mt-2 italic">If nothing shows above, no one has a birthday today or in the next 7 days (or no date_of_birth is set for any user).</p>
      </section>
    </div>
  )
}
